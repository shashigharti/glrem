...
class S1(tqdm_joblib):
    ...
    orbits_url = (
        "https://s1orbits.pechnikov.workers.dev/{mission}/{year}/{month:02}/{day:02}/"
    )
    ...
    def download_orbits(
        ...
        if isinstance(scenes, pd.DataFrame):
            if skip_exist:
                ...
                df_data = scenes[scenes.orbitpath.isnull()][
                    ["mission", "datetime"]
                ].values
            else:
            ...
            df_data = [
                (
                    scene.split("_")[0],
                    datetime.strptime(scene.split("_")[5], "%Y%m%dT%H%M%S"),
                )
                for scene in scenes
            ]
        else:
        ...
        def download_index(mission, dt, orbit_offset_start, orbit_offset_end):
            ...
            with requests.get(url + "index.csv", timeout=S1.http_timeout) as response:
                ...
                orbits["product"] = orbits["orbit"].apply(
                    lambda name: name.split("_")[3]
                )
                orbits["mission"] = orbits["orbit"].apply(lambda name: name[:3])
                orbits["time"] = orbits["orbit"].apply(
                    lambda name: datetime.strptime(name.split("_")[5], "%Y%m%dT%H%M%S")
                )
            ...
            for orbit in orbits.itertuples():
                ...
                if orbit.product == "RESORB":
                    ...
                    time_start = (
                        datetime.strptime(orbit_parts[6][1:], "%Y%m%dT%H%M%S")
                        + orbit_offset_start
                    )
                    time_end = (
                        datetime.strptime(orbit_parts[7], "%Y%m%dT%H%M%S")
                        - orbit_offset_end
                    )
            ...
            raise ValueError(
                f"Orbit product not found for mission {mission} and timestamp {dt}"
            )
            ...
            with requests.get(url, timeout=S1.http_timeout) as response:
                ...
                with open(filename, "wb") as f:
                    with zipfile.ZipFile(BytesIO(response.content), "r") as zip_in:
                        ...
                        if len(zip_files) > 1:
                            raise Exception(
                                "NOTE: Downloaded zip archive includes multiple files."
                            )
        ...
        with S1.tqdm_joblib(
            tqdm(desc="Downloading Sentinel-1 Orbits Index:", total=len(df))
        ) as progress_bar:
            orbits = joblib.Parallel(n_jobs=n_jobs, backend=joblib_backend)(
                joblib.delayed(download_index)(
                    scene.mission,
                    scene.datetime,
                    S1.orbit_offset_start,
                    S1.orbit_offset_end,
                )
                for scene in df.itertuples()
            )
        ...
        # download orbits index files and detect the orbits
        with S1.tqdm_joblib(
            tqdm(desc="Downloading Sentinel-1 Orbits:", total=len(orbits))
        ) as progress_bar:
            joblib.Parallel(n_jobs=n_jobs, backend=joblib_backend)(
                joblib.delayed(download_orbit)(basedir, orbit.url + orbit.orbit)
                for orbit in orbits.itertuples()
            )
        return orbits["orbit"]
    ...
    def scan_slc(
        ...
        assert (
            orbit is None or orbit == "A" or orbit == "D"
        ), "ERROR: use symbol A (Ascending) or D (Descending) for orbit filter"
        assert (
            mission is None or mission == "S1A" or mission == "S1B"
        ), "ERROR: use name S1A or S1B for mission filter"
        assert subswath is None or subswath in [
            1,
            2,
            3,
            12,
        ...
        assert polarization is None or polarization in [
            "VV",
            "VH",
            "HH",
            "HV",
        ...
        if mission is not None:
            path_pattern = (
                f"{mission.lower()}-iw{subswath}-slc-{polarization.lower()}-*"
            )
        else:
        ...
        if calibration:
            noisenames = [
                os.path.splitext(os.path.split(path)[-1])[0][6:] for path in noisepaths
            ]
            # print ('noisenames', noisenames)
            calibnames = [
                os.path.splitext(os.path.split(path)[-1])[0][12:] for path in calibpaths
            ]
        ...
        if calibration:
            ...
            assert (
                noisenames == calibnames
            ), "Found inconsistent set of calibration .xml files"
        ...
        orbitnames = [
            os.path.splitext(os.path.split(path)[-1])[0] for path in orbitpaths
        ]
        if orbitpaths:
            orbit_dates = [
                (text2date(name.split("_")[-2]), text2date(name.split("_")[-1]))
            ...
            orbits = [
                orbits.get((date - oneday, date + oneday))
                or orbits.get((date - oneday, date))
                or orbits.get((date, date))
                for date in ds
            ]
        ...
        orbits = [
            S1.read_annotation(path)["product"]["generalAnnotation"][
                "productInformation"
            ]["pass"][:1]
        ...
        if not len(missions) == 0:
            raise ValueError(
                "ERROR: Found multiple scenes for a single date from different missions"
            )
        ...
        if not int(subswaths) in [1, 2, 3, 12, 23, 123]:
            raise ValueError(
                f"ERROR: Subswhats list {subswaths} incorrect. Allowed a single or sequential subswath numbers 1, 2, 3, 12, 23, 123"
            )
        if not len(df.orbit.unique()) <= 1:
            raise ValueError(
                "ERROR: Only single orbit processing supported. Use any one ascending or descending"
            )
    ...
    def geoloc2bursts(metapath):
        ...
        grouped_lines = df.groupby("line_change")["geometry"].apply(
    ...
    def get_geoloc(annotation):
        ...
        return gpd.GeoDataFrame(
            gcps, geometry=gpd.points_from_xy(x=gcps.longitude, y=gcps.latitude)
        )
