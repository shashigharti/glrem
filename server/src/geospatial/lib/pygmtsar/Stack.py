...
class Stack(Stack_export):
    ...
    def __init__(self, basedir, drop_if_exists=False):
        ...
        if os.path.exists(basedir):
            if drop_if_exists:
                ...
                raise ValueError(
                    "ERROR: The base directory already exists. Use drop_if_exists=True to delete it and start new processing."
                )
        os.makedirs(basedir)
    ...
    def set_scenes(self, scenes):
        ...
        assert (
            len(scenes[scenes.orbitpath.isna()]) == 0
        ), 'ERROR: orbits missed, check "orbitpath" column.'
        self.df = scenes
        if self.reference is None:
            print(
                f'NOTE: auto set reference scene {scenes.index[0]}. You can change it like Stack.set_reference("2022-01-20")'
            )
    ...
    def plot_scenes(
        ...
        **kwargs,
        ...
        overlap_count = [
            sum(1 for geom2 in gdf.geometry if geom1.intersects(geom2))
        ...
        gdf.reset_index().plot(
            color=[colors[k] for k in gdf.index],
            alpha=max(1 / max(overlap_count), 0.002),
            edgecolor="black",
            ax=plt.gca(),
        )
    ...
    def plots_POI(fg, geometry=None, **kwargs):
        ...
        def plot_geopandas_on_subplot():
            ...
            ax = plt.gca()
            geometry.plot(
                ax=ax, marker=marker, markersize=marker_size, color=marker_color
            )
            # fix subplot aspect
            ax.set_aspect("auto")
