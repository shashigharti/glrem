...
class GMT:
    def download_dem(self, geometry, filename=None, product="1s", skip_exist=True):
        ...
        """
        print(
            "NOTE: GMT module is removed. Download DEM using Tiles().download_dem(AOI, filename=...)."
        )

    def download_landmask(
        self, geometry, filename=None, product="1s", resolution="f", skip_exist=True
    ):
        """
        ...
        """
        print(
            "NOTE: GMT module is removed. Download landmask using Tiles().download_landmask(AOI, filename=...)."
        )
