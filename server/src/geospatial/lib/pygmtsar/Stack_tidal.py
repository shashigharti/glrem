...
class Stack_tidal(Stack_incidence):
    ...
    def tidal_los(self, stack):
        ...
        def interp_block(pair, ys_block, xs_block):
            ...
            coords = {"y": ys_block, "x": xs_block}
            block_tidal1 = (
                solid_tide.sel(date=date1)
                .interp(coords, method="linear", assume_sorted=True)
                .compute(n_workers=1)
            )
            block_tidal2 = (
                solid_tide.sel(date=date2)
                .interp(coords, method="linear", assume_sorted=True)
                .compute(n_workers=1)
            )
            block_look = sat_look.interp(
                coords, method="linear", assume_sorted=True
            ).compute(n_workers=1)
            block_tidal = block_tidal2 - block_tidal1
            los = xr.dot(
                xr.concat(
                    [block_look.look_E, block_look.look_N, block_look.look_U], dim="dim"
                ),
                xr.concat([block_tidal.dx, block_tidal.dy, block_tidal.dz], dim="dim"),
                dims=["dim"],
            )
        ...
        ys_blocks = np.array_split(
            grid.y, np.arange(0, grid.y.size, self.chunksize)[1:]
        )
        xs_blocks = np.array_split(
            grid.x, np.arange(0, grid.x.size, self.chunksize)[1:]
        )
            ...
            for ys_block in ys_blocks:
                ...
                for xs_block in xs_blocks:
                    block = dask.array.from_delayed(
                        dask.delayed(interp_block)(pair, ys_block, xs_block),
                        shape=(1, ys_block.size, xs_block.size),
                        dtype=np.float32,
                    )
                    blocks.append(block)
                    del block
    ...
    def _tidal(self, date, grid):
        ...
        SC_clock_start, SC_clock_stop = self.PRM_merged(date).get(
        ...
        out = np.fromstring(stdout_data, dtype=np.float32, sep=" ").reshape(
            grid.y.size, grid.x.size, 5
        )[
            None,
        ]
        coords = {"date": pd.to_datetime([date]), "y": grid.y, "x": grid.x}
        das = {
            v: xr.DataArray(out[..., idx], coords=coords)
            for (idx, v) in enumerate(["lon", "lat", "dx", "dy", "dz"])
        }
        ds = xr.Dataset(das)
        return ds
    ...
    def compute_tidal(self, dates=None, coarsen=32, n_jobs=-1, interactive=False):
        ...
        if step_x < 1:
            step_x = 1
        grid = trans_inv.sel(
            y=trans_inv.y[step_y // 2 :: step_y], x=trans_inv.x[step_x // 2 :: step_x]
        )
        ...
        with self.tqdm_joblib(
            tqdm(desc="Tidal Computation", total=len(dates))
        ) as progress_bar:
            outs = joblib.Parallel(n_jobs=n_jobs)(
                joblib.delayed(tidal)(date) for date in dates
            )
    ...
    def plot_tidal(
        self,
        data=None,
        caption="Tidal Displacement Amplitude, [mm]",
        cmap="turbo",
        cols=4,
        size=4,
        nbins=5,
        ...
        for i, date in enumerate(data.date.values):
            ...
            im = date_data.plot.imshow(ax=axes[i], cmap=cmap, add_colorbar=False)
    ...
    def plot_tidal_los_rad(
        self,
        data=None,
        caption="Tidal Phase, [rad]",
        cmap="turbo",
        cols=4,
        size=4,
        nbins=5,
