...
class Stack_incidence(Stack_geocode):
    ...
    def los_projection(self, data):
        ...
        if isinstance(data, xr.Dataset):
            ...
            los = xr.dot(
                xr.concat([look.look_E, look.look_N, look.look_U], dim="dim"),
    ...
    def incidence_angle(self):
        ...
        sat_look = self.get_satellite_look_vector()
        incidence_ll = np.arctan2(
            np.sqrt(sat_look.look_E**2 + sat_look.look_N**2), sat_look.look_U
        ).rename("incidence_angle")
        return incidence_ll

    def plot_incidence_angle(
        self,
        data="auto",
        caption="Incidence Angle in Radar Coordinates, [rad]",
        cmap="gray",
        aspect=None,
        **kwargs
    ):
    ...
    def vertical_displacement_mm(self, data):
        ...
        assert self.is_ra(
            data
        ), "ERROR: argument data needs to be defined in radar coordinates"
    ...
    def elevation_m(self, data, baseline=1):
        ...
        assert self.is_ra(
            data
        ), "ERROR: unwrapped phase needs to be defined in radar coordinates"

        # expected accuracy about 0.01%
        # wavelength, slant_range = self.PRM().get('radar_wavelength','SC_height')
        wavelength, slant_range_start, slant_range_end = self.PRM_merged().get(
            "radar_wavelength", "SC_height_start", "SC_height_end"
        )

        incidence_angle = self.incidence_angle()
        slant_range = xr.DataArray(
            np.linspace(slant_range_start, slant_range_end, incidence_angle.shape[1]),
        ...
        return -(
            wavelength * data * slant * np.cos(incidence) / (4 * np.pi * baseline)
        ).rename("ele")
