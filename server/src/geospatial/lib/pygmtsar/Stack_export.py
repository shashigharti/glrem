...
class Stack_export(Stack_ps):
    ...
    def as_vtk(dataset):
        ...
        assert isinstance(
            dataset, xr.Dataset
        ), 'ERROR: Expected "dataset" argument as Xarray Dataset'
            ...
            if "band" in dims:
                ...
                if bands in [3, 4]:
                    ...
                    array = vn.numpy_to_vtk(
                        da[:3].round().astype(np.uint8).values.reshape(3, -1).T,
                        deep=True,
    ...
    def export_geotiff(
        self, data, name, caption="Exporting WGS84 GeoTIFF(s)", compress="LZW"
    ):
        """
        ...
        assert isinstance(
            data, xr.DataArray
        ), "Argument data is not an xr.DataArray object"
        ...
        with tqdm(
            desc=caption, total=len(data[stackvar]) if stackvar is not None else 1
        ) as pbar:
            for grid in (
                data.transpose(stackvar, ...) if stackvar is not None else [data]
            ):
                # convert the stack variable value to a string suitable for filenames
                if stackvar is not None and np.issubdtype(
                    grid[stackvar].dtype, np.datetime64
                ):
                ...
                filename = (
                    f"{name}.{stackval}.tif" if stackvar is not None else f"{name}.tif"
                )
                # print ('filename', filename)
                # convert the data to geographic coordinates if necessary and export to GeoTIFF
                self.as_geo(
                    self.ra2ll(grid) if not self.is_geo(grid) else grid
                ).rio.to_raster(filename, compress=compress)
                pbar.update(1)
    ...
    def export_geojson(
        ...
        data,
        name,
        caption="Exporting WGS84 GeoJSON",
        pivotal=True,
        digits=2,
        coord_digits=6,
    ):
        ...
        def block_as_json(block, stackvar, name):
            ...
            if stackvar is not None and np.issubdtype(
                df[stackvar].dtype, np.datetime64
            ):
            ...
            gdf = gpd.GeoDataFrame(
                ...
                geometry=gpd.points_from_xy(
                    df.lon.round(coord_digits), df.lat.round(coord_digits)
                ),
            )
        ...
        with open(f"{name}.geojson", "w") as f:
            ...
            if "stack" in data.dims:
                stack_blocks = np.array_split(
                    np.arange(grid["stack"].size),
                    np.arange(0, grid["stack"].size, self.chunksize1d)[1:],
                )
                # prepare the progress bar
                ...
                lats_blocks = np.array_split(
                    np.arange(grid.lat.size),
                    np.arange(0, grid.lat.size, self.netcdf_chunksize // 2)[1:],
                )
                lons_blocks = np.array_split(
                    np.arange(grid.lon.size),
                    np.arange(0, grid.lon.size, self.netcdf_chunksize // 2)[1:],
                )
                # prepare the progress bar
                with tqdm(
                    desc=caption, total=len(lats_blocks) * len(lons_blocks)
                ) as pbar:
    ...
    def export_csv(
        ...
        with open(f"{name}.csv", "w") as f:
            # CSV header
            f.write(
                delimiter.join(filter(None, [stackvar, "lon", "lat", data.name])) + "\n"
            )
            if "stack" in data.dims:
                stack_blocks = np.array_split(
                    np.arange(grid["stack"].size),
                ...
                with tqdm(desc=caption, total=len(stack_blocks)) as pbar:
                    for stack_block in stack_blocks:
                        block = (
                            grid.isel(stack=stack_block).drop_vars(["y", "x"]).compute()
                        )
                        ...
                        if stackvar is not None:
                            ...
                            block_csv = np.column_stack(
                                (
                                    np.repeat(stackvals, block_lon.size),
                            ...
                            block_csv = np.column_stack(
                                (block_lon, block_lat, block_val.astype(str).ravel())
                            )
                    ...
                    for lats_block in lats_blocks:
                        for lons_block in lons_blocks:
                            ...
                            if stackvar is not None:
                                ...
                                if np.issubdtype(stackvals.dtype, np.datetime64):
                                    stackvals = stackvals.dt.date.astype(str)
                                stackvals, lats, lons = np.meshgrid(
                                    stackvals, block_lat, block_lon, indexing="ij"
                                )
                                block_csv = np.column_stack(
                                    (
                                        stackvals.ravel(),
                                ...
                                lats, lons = np.meshgrid(
                                    block_lat, block_lon, indexing="ij"
                                )
                                block_csv = np.column_stack(
                                    (
                                        lons.ravel(),
                                        lats.ravel(),
                                        block_val.astype(str).ravel(),
                                    )
                                )
                                del lats, lons
                            del block, block_lat, block_lon
    ...
    def export_netcdf(
        ...
        data,
        name,
        caption="Exporting WGS84 NetCDF",
        engine="netcdf4",
        ...
        if "stack" in data.dims and isinstance(
            ...
            print(
                f"NOTE: open as xr.open_dataarray('{name}.nc').set_index(stack=['lat', 'lon'])"
            )
        ...
        delayed = grid.to_netcdf(
            filename, engine=engine, encoding=encoding, format=format, compute=False
        )
    ...
    def export_vtk(
        ...
        assert data is None or isinstance(
            data, xr.DataArray
        ), "Argument data is not an xr.DataArray object or None"
        assert (
            data is not None or image is not None
        ), 'One of arguments "data" or "image" needs to be specified'
