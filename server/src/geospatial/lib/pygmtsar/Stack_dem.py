...
class Stack_dem(Stack_reframe):
    ...
    def get_geoid(self, grid=None):
        ...
        with resources.as_file(
            resources.files("pygmtsar.data") / "geoid_egm96_icgem.grd"
        ) as geoid_filename:
            geoid = xr.open_dataarray(
                geoid_filename, engine=self.netcdf_engine, chunks=self.netcdf_chunksize
            ).rename({"y": "lat", "x": "lon"})
        if grid is not None:
    ...
    def set_dem(self, dem_filename):
        ...
        if dem_filename is not None:
            ...
            assert os.path.isfile(dem_filename) and os.access(
                dem_filename, os.R_OK
            ), f"DEM file is not readable: {dem_filename}"
    ...
    def get_dem(self):
        ...
        dem = xr.open_dataset(
            self.dem_filename, engine=self.netcdf_engine, chunks=self.chunksize
        )
        ...
        z_array_name = [
            ...
            if len(dem.data_vars[data_var].coords) == 2
        ]
        ...
        return dem.transpose("lat", "lon").sel(
            lat=slice(bounds[1] - self.buffer_degrees, bounds[3] + self.buffer_degrees),
    ...
    def load_dem(self, data, geometry="auto", buffer_degrees=None):
        ...
        if isinstance(data, (xr.Dataset)):
            ...
            ortho = data
        elif isinstance(data, str) and os.path.splitext(data)[-1] in [
            ".tiff",
            ".tif",
            ".TIF",
        ]:
            ortho = (
                rio.open_rasterio(data, chunks=self.chunksize)
                .squeeze(drop=True)
                .rename({"y": "lat", "x": "lon"})
                .drop("spatial_ref")
            )
            ...
            ortho = xr.open_dataarray(
                data, engine=self.netcdf_engine, chunks=self.chunksize
            )
        elif isinstance(data, str):
            print(
                "ERROR: filename extension is not recognized. Should be one from .tiff, .tif, .TIF, .nc, .netcdf, .grd"
            )
        else:
        ...
        duplicates = (
            lat_index[lat_index.duplicated()].tolist()
            + lon_index[lon_index.duplicated()].tolist()
        )
        assert (
            len(duplicates) == 0
        ), "ERROR: DEM grid includes duplicated coordinates, possibly on merged tiles edges"
        ...
        ortho = ortho.transpose("lat", "lon").sel(
            lat=slice(bounds[1] - self.buffer_degrees, bounds[3] + self.buffer_degrees),
            lon=slice(bounds[0] - self.buffer_degrees, bounds[2] + self.buffer_degrees),
        )
