...
class Stack_prm(Stack_base):
    ...
    def prm_offsets(self, debug=False):
        ...
        if len(subswaths) == 1:
            ...
            maxx, yvalid, num_patch = prm.get(
                "num_rng_bins", "num_valid_az", "num_patches"
            )
        ...
        assert len(
            np.unique([prm.get("PRF") for prm in prms])
        ), "Image PRFs are not consistent"
        assert len(
            np.unique([prm.get("rng_samp_rate") for prm in prms])
        ), "Image range sampling rates are not consistent"

        bottoms = [0] + [
            int(
                np.round(
                    (
                        (prm.get("clock_start") - prms[0].get("clock_start"))
                        * 86400
                        * prms[0].get("PRF")
                    )
                )
        ...
        # ovl12,23: 2690, 2558
        ovls = [
            prm1.get("num_rng_bins")
            - int(
                np.round(
                    (
                        (prm2.get("near_range") - prm1.get("near_range"))
                        / (constants.speed_of_light / prm1.get("rng_samp_rate") / 2)
                    )
                )
            )
        ...
        # for SLC
        maxy = max(
            [prm.get("num_valid_az") + bottom for prm, bottom in zip(prms, bottoms)]
        )
        if debug:
            print("maxy", maxy)
        maxx = sum(
            [prm.get("num_rng_bins") - ovl - 1 for prm, ovl in zip(prms, [-1] + ovls)]
        )
        if debug:
            print("maxx", maxx)
        ...
        ns = [
            np.ceil(-prm.get("rshift") + prm.get("first_sample") + 150.0).astype(int)
            for prm in prms[1:]
        ]
