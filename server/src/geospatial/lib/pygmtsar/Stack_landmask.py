...
class Stack_landmask(Stack_multilooking):
    ...
    def set_landmask(self, landmask_filename):
        ...
        if landmask_filename is not None:
            assert os.path.exists(
                landmask_filename
            ), f"Landmask file not found: {landmask_filename}"
            assert os.path.isfile(landmask_filename) and os.access(
                landmask_filename, os.R_OK
            ), f"Landmask file is not readable: {landmask_filename}"
    ...
    def get_landmask(self):
        ...
        landmask = xr.open_dataset(
            self.landmask_filename, engine=self.netcdf_engine, chunks=self.chunksize
        )
        assert "lat" in landmask.coords and "lon" in landmask.coords
        # define latlon array
        z_array_name = [
            data_var
            for data_var in landmask.data_vars
            if len(landmask.data_vars[data_var].coords) == 2
        ]
    ...
    def download_landmask(self, product="1s", debug=False):
        print(
            "NOTE: Function is removed. Download land mask using Tiles().download_landmask()"
        )
    ...
    def load_landmask(self, data, geometry="auto"):
        ...
        if self.landmask_filename is not None:
            print(
                "NOTE: landmask exists, ignore the command. Use Stack.set_landmask(None) to allow new landmask downloading"
            )
            return
        ...
        landmask.rename("landmask").load().to_netcdf(
            landmask_filename, encoding=encoding, engine=self.netcdf_engine
        )
