...
class datagrid:
        ...
        if isinstance(chunksize, (tuple, list, np.ndarray)):
            ...
            if shape is not None:
                assert len(shape) == len(
                    chunksize
                ), f"ERROR: defined shape and chunksize dimensions are not equal: {len(shape)} != {len(chunksize)}"
                chunksizes = tuple(
                    [
                        chunksize[dim] if chunksize[dim] < shape[dim] else shape[dim]
            ...
            if shape is not None:
                ...
                for idim in range(len(shape)):
                    chunksizes.append(
                        chunksize if chunksize < shape[idim] else shape[idim]
                    )
    ...
    def cropna(das, index=-1):
        ...
        assert (
            len(dims) == 2
        ), 'ERROR: the input should be 3D array with "pair" or "date" coordinate'
    ...
    def get_bounds(geometry):
        ...
        if isinstance(geometry, (xr.DataArray, xr.Dataset)) and (
            "lat" in geometry.dims and "lon" in geometry.dims
        ):
    ...
    def nearest_grid(self, in_grid, search_radius_pixels=None):
        ...
        assert (
            in_grid.chunks is not None
        ), "nearest_grid() input grid chunks are not defined"

        if search_radius_pixels is None:
            search_radius_pixels = self.chunksize
        elif search_radius_pixels <= 0:
            print(
                f"NOTE: interpolation ignored for search_radius_pixels={search_radius_pixels}"
            )
            return in_grid
        else:
            assert (
        ...
        def func(grid, y, x, distance, scaley, scalex):
            ...
            source_yxs = np.stack(
                [ys[~nanmask] / scaley, xs[~nanmask] / scalex], axis=1
            )
            ...
            target_yxs = np.stack(
                [
                    (y / scaley).reshape(-1)[nanmask0],
                    (x / scalex).reshape(-1)[nanmask0],
                ],
                axis=1,
            )
            # assert 0, target_yxs
            d, inds = tree.query(
                target_yxs, k=1, distance_upper_bound=distance, workers=1
            )
        ...
        grid = xr.apply_ufunc(
            ...
            dask_gufunc_kwargs={
                "distance": search_radius_pixels,
                "scaley": scale[0],
                "scalex": scale[1],
            },
        )
        assert (
            grid.chunks is not None
        ), "nearest_grid() output grid chunks are not defined"
        return grid
    ...
    def get_coarsen(self, factor):
        ...
        if coarsens[1] > 4 * psizes[1]:
            coarsens = np.round(
                [coarsens[0] / psizes[0], coarsens[1] / psizes[1] / 4]
            ).astype(int)
            coarsens[1] = 4 * coarsens[1]
        else:
            coarsens = np.round(
                [coarsens[0] / psizes[0], coarsens[1] / psizes[1]]
            ).astype(int)
