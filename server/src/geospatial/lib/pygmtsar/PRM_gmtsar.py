...
class PRM_gmtsar:
    ...
    def calc_dop_orb(
        self, earth_radius=0, doppler_centroid=0, inplace=False, debug=False
    ):
        """
    ...
    def SAT_llt2rat(
        ...
        if coords is not None and fromfile is None:
            ...
            raise Exception(
                "Should be defined data source as coordinates triplet (coords) or as file (fromfile)"
            )
        ...
        if stderr_data.startswith("interpolation point outside of data constraints"):
            print(
                "Error: SAT_llt2rat processing stopped due to invalid coordinates for one of input points"
            )
            return None
    ...
    def SAT_look(
        self, coords=None, fromfile=None, tofile=None, binary=False, debug=False
    ):
        """
