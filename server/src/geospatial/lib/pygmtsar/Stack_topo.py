...
class Stack_topo(Stack_trans_inv):
    ...
    def plot_topo(
        self,
        data="auto",
        caption="Topography on WGS84 ellipsoid, [m]",
        quantile=None,
        vmin=None,
        vmax=None,
        symmetrical=False,
    ...
    def topo_phase(self, pairs, topo="auto", grid=None, method="nearest", debug=False):
        ...
        def calc_drho(rho, topo, earth_radius, height, b, alpha, Bx):
            ...
            cost = (rho**2 + c**2 - ret**2) / (2.0 * rho * c)
            # if (cost >= 1.)
        ...
        def block_phase_dask(block_topo, y_chunk, x_chunk, prm1, prm2):
            ...
            Bh0 = prm2.get("baseline_start") * np.cos(
                prm2.get("alpha_start") * np.pi / 180
            )
            Bv0 = prm2.get("baseline_start") * np.sin(
                prm2.get("alpha_start") * np.pi / 180
            )
            ...
            if (
                prm2.get("baseline_center") != 0
                or prm2.get("alpha_center") != 0
                or prm2.get("B_offset_center") != 0
            ):
                Bhc = prm2.get("baseline_center") * np.cos(
                    prm2.get("alpha_center") * np.pi / 180
                )
                Bvc = prm2.get("baseline_center") * np.sin(
                    prm2.get("alpha_center") * np.pi / 180
                )
        ...
        def prepare_prms(pair, offsets):
            ...
            prm1.set(
                prm1.SAT_baseline(prm1).sel(
                    "SC_height", "SC_height_start", "SC_height_end"
                )
            ).fix_aligned()
            return (prm1, prm2)

        prms = joblib.Parallel(n_jobs=-1)(
            joblib.delayed(prepare_prms)(pair, offsets) for pair in pairs
        )
        ...
        return (
            xr.DataArray(out, coords={"pair": coord_pair, "y": topo.y, "x": topo.x})
            .assign_coords(ref=coord_ref, rep=coord_rep, pair=coord_pair)
            .where(da.isfinite(topo))
            .rename("phase")
        )
    ...
    def topo_slope(self, topo="auto", edge_order=1):
        ...
        slope = xr.DataArray(
            slope_degrees, coords=topo.coords, dims=topo.dims, name="slope_degrees"
        )
        slope.attrs["units"] = "degrees"
        slope.attrs["description"] = (
            "Slope calculated from DEM, accounting for non-uniform grid spacing"
        )
