...
class MultiInstanceManager:
        ...
        for key, value in kwargs.items():
            if isinstance(value, collections.abc.Iterable) and not isinstance(
                ...
                if len(value_list) != len(self.instances):
                    raise ValueError(
                        f"Expected a list of {len(self.instances)} values for '{key}', but got {len(value_list)}"
                    )
                self.context_params[key] = value_list
            else:
                raise ValueError(
                    f"Provided value for '{key}' is not an appropriate iterable"
                )
        return self
    ...
    def run_callable(self, func):
        ...
        for instance in self.instances:
            instance_args = {
                k: v[self.instances.index(instance)]
        ...
        for instance in self.instances:
            if hasattr(instance, method_name):
                ...
                raise AttributeError(
                    f"{instance} does not have a method named {method_name}"
                )
        return results
